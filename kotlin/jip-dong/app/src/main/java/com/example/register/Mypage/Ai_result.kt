...
class Ai_result : AppCompatActivity() {
    ...
    private lateinit var highestSimilarityUser: String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        button2.setOnClickListener {
            // 다른 액티비티 실행
            val intent = Intent(this,Ai_result_2::class.java)
            startActivity(intent)
        }

        button1 = findViewById<View>(R.id.button1) as Button;
        button1.setOnClickListener {
            val user2 = highestSimilarityUser
            val sharedPreferences: SharedPreferences = getSharedPreferences("login", Context.MODE_PRIVATE)
            val uid = sharedPreferences.getString("uid", "")
            val url2 = "http://capstone123.dothome.co.kr/getusername.php?uid=$user2"
                    ...
                    if (response == "이미 생성된 채팅방") {
                        ...
                        val getChatRoomIdRequest = object : StringRequest(
                            ...
                            Response.Listener<String> { roomId ->
                                val intent = Intent(this@Ai_result, Chatting::class.java)
    ...
    inner class RequestTask : AsyncTask<String, Void, String>() {
        ...
        override fun onPostExecute(result: String) {
            try {
                ...
                highestSimilarityUser = jsonResult.getString("highestSimilarityUser")
                val highestSimilarity = jsonResult.getDouble("highestSimilarity")
                val formattedSimilarity = String.format("%.2f", highestSimilarity * 100)

                val url3 = "http://capstone123.dothome.co.kr/getusername.php?uid=$highestSimilarityUser"
